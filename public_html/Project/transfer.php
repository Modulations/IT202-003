    ...
    try {
        ...
        if (count($res) == 0) {
            ...
            try {
                $negativeone = -1;
                $res = 0;
                $res = $stmt->execute([":acctSrc" => $acct_src, ":acctDest" => $acct_dest, ":balance_change" => (intval($balance_change) * $negativeone), ":transactionType" => "withdraw", ":memo" => $memo, ":expectedTotal" => $userBalance]);
                $res = $stmt->execute([":acctSrc" => $acct_dest, ":acctDest" => $acct_src, ":balance_change" => intval($balance_change), ":transactionType" => "withdraw", ":memo" => $memo, ":expectedTotal" => $destNewBalance]);
                flash("Success!");
            } catch (Exception $e) {
                flash($e . $res);
            }
